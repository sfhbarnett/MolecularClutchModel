...
ksub = 2e-3 #substrate stiffness, 0.01-100 pN/nm, = 0.01e-3-100e-3 N/m
...
for i in range(timepoints):
    ...
    truevelocity = actin_flow * (1 - (ksub * xsub) / Fstall)
    ...
    SubstrateForce.append(xsub*ksub)
    #Update the force on each clutch for the new substrate position
    force = [kclutch*(abs(cp-xsub)) for cp in clutch_position]
